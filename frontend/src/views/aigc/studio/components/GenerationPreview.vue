...
<template>
  <div class="generation-preview">
    ...
    <div v-if="!loading && !result && !task" class="generation-preview__empty">
      ...
      <div class="generation-preview__empty-desc"> 描述你想要的内容，AI会自动理解并为你创作 </div>
    ...
    <div v-else-if="loading" class="generation-preview__loading">
      ...
      <div class="generation-preview__loading-info">
        ...
        <div v-if="task?.agentAnalysis" class="generation-preview__agent-decision">
          ...
          <div class="decision-item">
            ...
            <el-tag size="small" type="success" effect="plain">{{
              task.agentAnalysis.selectedModel
            }}</el-tag>
          </div>
          <div class="decision-item">
            <span class="label">输出类型</span>
            <el-tag size="small" type="warning" effect="plain">{{
              getContentTypeLabel(task.agentAnalysis.contentType)
            }}</el-tag>
          </div>
          <div v-if="task.agentAnalysis.confidence" class="decision-item">
            <span class="label">置信度</span>
            <el-tag size="small" type="info" effect="plain">{{
              formatConfidence(task.agentAnalysis.confidence)
            }}</el-tag>
          </div>
        </div>
        ...
        <div
          ...
          class="generation-preview__optimized-prompt"
        >
          <div class="prompt-label">
            <el-icon><MagicStick /></el-icon>
            优化后的提示词
    ...
    <div v-else-if="isFailed" class="generation-preview__failed">
      ...
      <div class="generation-preview__actions">
        <el-button type="primary" :icon="Refresh" @click="handleRetry"> 重试任务 </el-button>
        <el-button
          v-if="task?.agentAnalysis?.optimizedPrompt"
          :icon="Edit"
    ...
    <div v-else-if="result" class="generation-preview__result">
      ...
      <div
        v-if="providerExecutionItems.length"
        class="generation-preview__provider-observe generation-preview__provider-observe--result"
      >
        <div v-for="item in providerExecutionItems" :key="item.label" class="observe-item">
          <span>{{ item.label }}</span>
          <strong>{{ item.value }}</strong>
      ...
      <div
        ...
        class="generation-preview__references generation-preview__references--result"
      >
        <div class="generation-preview__references-title">参考素材</div>
        <div class="generation-preview__references-list">
          <button
