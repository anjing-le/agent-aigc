...
| 素材库 | V0.9 | 已提供上传 API、素材记录、列表/删除 API、前端素材库、任务引用 ID 记录、结果区引用展示和基础引用统计；后续补清理和权限 |
...
- 素材库已具备上传、记录、列表、预览、删除、任务引用 ID 记录、引用展示和按素材反查任务接口，但存储清理和权限隔离仍待产品化。
...
2. 做素材库：存储清理、本地/OSS 存储抽象、素材与任务详情联动。
