...
| 素材上传 | Planned | 前端支持参考图 base64 传入，后续需要独立上传、存储、素材库和权限 |
...
- 素材上传仍是轻量传参，没有独立上传 API、文件校验、存储清理和权限隔离。
- Provider 配置还缺少管理页，当前主要通过环境变量切换。
...
2. 做素材库：上传 API、本地/OSS 存储抽象、素材预览、引用关系。
