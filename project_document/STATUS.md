...
- 素材库和资产库已具备统一 storage adapter、OSS SDK 上传/删除、本地文件清理、存储状态诊断、OSS 短重试、清理审计、审计查询视图、用户/租户可见性隔离、授权下载、授权预览和静态 `/files/**` 兼容开关；历史文件归属迁移和公共广场资源策略仍待产品化。
...
1. 做存储产品化：历史文件归属迁移、公共广场资源策略和生产环境资源访问策略。
