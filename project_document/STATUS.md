...
| 素材库 | V0.9 | 已提供上传 API、素材记录、列表/删除 API、前端素材库、任务引用 ID 记录和结果区引用展示；后续补反查、清理和权限 |
...
- 新增 AIGC 素材上传与列表 API，前端创作台引用参考图时不再传 base64，素材库页面可预览、筛选、删除和复制素材 URL，任务状态与结果区可回看引用素材。
...
- 素材库已具备上传、记录、列表、预览、删除、任务引用 ID 记录和引用展示，但按素材反查、存储清理和权限隔离仍待产品化。
