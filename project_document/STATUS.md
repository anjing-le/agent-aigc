...
| 素材库 | V0.9 | 已提供上传 API、素材记录、列表/删除 API、前端素材库、任务引用 ID 记录、结果区引用展示、基础引用统计、引用任务预览和本地文件清理；后续补 OSS adapter 和权限隔离 |
...
- 素材库和资产库已具备本地文件清理和存储状态诊断；真正 OSS adapter、OSS 清理、文件权限隔离和清理审计仍待产品化。
...
1. 做存储产品化：接入真正 OSS adapter、文件权限隔离、清理审计。
