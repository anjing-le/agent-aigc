...
| 素材上传 | V0.9 | 已提供独立上传 API，创作台参考素材先上传为后端托管 URL；后续补素材库、校验、清理和权限 |
...
- 新增 AIGC 素材上传 API，前端创作台引用参考图时不再传 base64，而是先上传并传入后端托管 URL。
...
- 素材上传已具备 API 和本地保存入口，但文件校验、素材库、存储清理和权限隔离仍待产品化。
...
2. 做素材库：素材预览、删除、引用关系、文件校验、本地/OSS 存储抽象。
