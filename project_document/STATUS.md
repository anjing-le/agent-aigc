...
| 素材库 | V0.9 | 已提供上传 API、素材记录、列表/删除 API、前端素材库和任务引用 ID 记录；后续补引用展示、清理和权限 |
...
- 素材库已具备上传、记录、列表、预览、删除和任务引用 ID 记录，但引用展示、按素材反查、存储清理和权限隔离仍待产品化。
...
2. 做素材库：引用展示、按素材反查、存储清理、本地/OSS 存储抽象。
