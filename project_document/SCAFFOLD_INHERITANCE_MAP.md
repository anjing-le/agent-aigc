...
AIGC 私有资源归属也从脚手架上下文生长：`AigcOwnershipService` 统一读取 `GlobalRequestContextHolder` 的 userId 和 tenantId，新上传素材、新生成任务、新生成资产写入 owner/tenant；任务、素材、资产和存储审计查询统一走 `findVisible...` 或规格查询过滤。历史空归属数据继续可见，用于兼容本地 demo 和早期迁移；需要治理时通过 `/api/aigc/ownership/backfill` 进入 `AigcOwnershipBackfillService`，先 dry-run 统计资产、素材、任务候选行，只有管理角色且显式确认后才按当前请求上下文回填 owner/user/tenant。这个接口同样从 `ApiConstants.Aigc`、`service-boundaries.json`、OpenAPI 类型和 `openApiRequest` 派生，教学时可以作为“历史数据迁移也继承脚手架权限与契约习惯”的例子。
