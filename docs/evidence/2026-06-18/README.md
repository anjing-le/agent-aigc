...
本目录记录 2026-06-18 对灵感广场“运营规则”、“人工运营专题”和“创作者榜单”的浏览器检查结论。截图文件仍建议放在课件素材仓库或本机素材目录，当前代码仓库只保留可追溯索引和检查结果。

## 基线

- 功能基线：`feat: add gallery editorial topics`、`feat: add gallery creator ranking`、`feat: expose gallery curation rules`
- 后端端口：`18190`
- 前端端口：`5179`
- 后端命令：`SPRING_PROFILES_ACTIVE=dev SERVER_PORT=18190 mvn -q spring-boot:run`
- 前端命令：`VITE_API_PROXY_URL=http://127.0.0.1:18190 pnpm dev --host 127.0.0.1 --port 5179`
- 数据准备：`./scripts/aigc-demo-smoke.sh http://127.0.0.1:18190`
...
- URL：`http://127.0.0.1:5179/#/aigc/gallery`
- 页面标题：`灵感广场 - Anjing AI`
- `人工运营专题` 可见。
- `运营规则` 可见。
- `rules v1` 可见。
...
- 专题、创作者榜单和运营规则继续沿用 `ApiConstants -> service-boundaries.json -> OpenAPI -> openApiRequest -> 页面` 的脚手架链路。
